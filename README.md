...
## 🛡️ امنیت و کارایی (Enterprise-Grade)

- **Advanced RBAC:** سیستم مدیریت سطح دسترسی پیشرفته. مدیران (Admin) می‌توانند ویراستاران (Editor) اختصاصی برای هر مقاله تعیین کنند. هر نقش فقط به مقالات مجاز خود دسترسی دارد. 
- **Interactive AI Improve:** چرخه‌ی بهبود هوش مصنوعی با پیش‌نمایش. امکان ارسال دستورالعمل اختصاصی به هوش مصنوعی و مقایسه نمره سئو (قبل و بعد) با تایید نهایی کاربر.
- **Dynamic Rate Limiting:** استفاده از `slowapi` برای دفع حملات Brute-force در لاگین و جلوگیری از استفاده بیش از حد از API هوش مصنوعی. سقف درخواست‌های روزانه از طریق پنل تنظیمات سیستم قابل تغییر است.
- **XSS Protection:** پاکسازی کامل کدهای HTML در سمت فرانت‌اند (با `dompurify`) و بک‌اند جهت خنثی‌سازی حملات تزریق کد.
- **Performance Optimization:** حذف Payload های سنگین HTML از لیست مقالات برای جلوگیری از OOM و کرش کردن سیستم.
- **Safe Authentication:** استفاده از `PyJWT` مدرن برای مدیریت توکن‌ها و جلوگیری از دسترسی‌های غیرمجاز با CORS Policy سخت‌گیرانه.
- **Soft Delete:** جلوگیری از از بین رفتن مقالات هنگام حذف کاربران. نگهداری کاربری که آخرین تغییرات را اعمال کرده است.
