...
# لنگرِ هویتِ فوق‌ادبی (Hyper-Persian Anchor) به جای تلهٔ منفی
PERSIAN_SYSTEM = (
    "تو یک نویسنده، ادیب و سردبیرِ تراز اولِ زبان فارسی در تحریریهٔ مجلهٔ معتبرِ «ترجمان علوم انسانی» هستی. "
    "نثر تو فاخر، اصیل، زنده، غیرتکراری و دارای پیوستگیِ ارگانیک است. "
    "تو به زبان فارسی معیار عشق می‌ورزی و سوگند یاد کرده‌ای که تحت هیچ شرایطی، حتی یک کلمه، شناسه یا اصطلاح از زبان‌های انگلیسی، آلمانی، عربیِ غلیظ، هندی یا آسیای شرقی در متنت وارد نکنی. "
    "برای اصطلاحاتِ جهانی، منحصراً از معادل‌های زندهٔ فارسی (مانند: دم‌آوری، عصاره‌گیری سرد، فیلتر، ظرف شیشه‌ای، آماده‌سازی) استفاده کن."
)

# جلادِ اسکریپت‌ها: شکارچیِ کاراکترهای هندی، کانجی، سیریلیک و اعراب‌های عجیبِ ویتنامی
_ALIEN_SCRIPTS = re.compile(
    r"[\u0900-\u097F"  # هندی / سانسکریت (Devanagari)
    r"\u4E00-\u9FFF\u3040-\u30FF\u31F0-\u31FF"  # کانجی / چینی / ژاپنی
    r"\u0E00-\u0E7F"  # تایلندی
    r"\u0400-\u04FF"  # روسی / سیریلیک
    r"ạảăắằẳẵặâấầẩẫậẹẻẽêềếểễệỉịọỏôốồổỗộơớờởỡợụủưứừửữựỵỷỹĐđ"  # حروف اختصاصیِ تن‌دارِ ویتنامی
    r"]+"
)

# کلماتِ بیگانهٔ شایعی که لاما ۷۰B عادت دارد وسطِ متنِ فارسی پرتاب کند
_KNOWN_FOREIGN_WORDS = re.compile(
    r"\b(drinking|prepare|zwischen|cold brew|filter|also|however|moreover|furthermore|phổ biến|coffee|moka|pot)\b",
    re.IGNORECASE,
)
...
def sanitize_html_for_tiptap(raw_html: str) -> str:
    """
    ۱. کاراکترهای جن‌زدهٔ شرقی و هندی را پودر می‌کند.
    ۲. تگ‌های من‌درآوردیِ LLM (مثل <container>) را باز می‌کند تا کلماتش بلعیده نشوند.
    """
    # گام اول: پاک‌سازی کلمات و الفبای بیگانه
    ...
    for tag in soup.find_all(True):
        if tag.name not in allowed_tags:
            # محتوای متنیِ تگ‌های ناشناخته را بیرون می‌کشد و خودِ تگ را می‌کشد
            tag.unwrap()
        ...
        messages = [
            {
                ...
                "content": (
                    PERSIAN_SYSTEM + " "
                    "تو یک سردبیرِ دانا و نویسندهٔ چیره‌دستِ زبان فارسی هستی. "
            ...
            },
            {
                ...
                "content": (
                    ...
                    "قوانینِ حیاتیِ نگارش (تخطی از این موارد موجب شکست پروژه است):\n"
                    "۱. ممنوعیت حشو و بدیهیات (Anti-Tautology): هرگز شناسه یا عنوانِ بخش را با خودش تعریف نکن! (مثلاً برای بخشِ «آسیاب کردن قهوه» ننویس: آسیاب کردن قهوه مرحله‌ای است که قهوه را آسیاب می‌کنیم). مخاطبِ تو باهوش است؛ مستقیماً وارد چراییِ علمی، تکنیک، درجهٔ آسیاب یا فوت کوزه‌گریِ آن مرحله شو.\n"
                    "۲. خلوصِ مطلقِ زبانی: متن باید ۱۰۰٪ فارسیِ پاک باشد. به جای واژگانِ لاتین مثل Cold Brew یا prepare یا Drinking، حتماً از معادل‌های زندهٔ فارسی (مانند: عصاره‌گیری سرد، آماده‌سازی، نوشیدن، عصاره) استفاده کن. نوشتنِ کلماتِ آلمانی، هندی یا شرقی جریمهٔ سنگین دارد!\n"
                    "۳. تنوع ریتم: ترکیبی متعادل از جملات کوتاه (۵ تا ۸ کلمه) و جملات مرکبِ زیبا (۱۵ تا ۲۰ کلمه) ایجاد کن تا متن آهنگِ طبیعیِ یک نویسندهٔ حرفه‌ای را پیدا کند.\n"
                    ...
                    "۵. ساختار بصری: بدنه را داخل تگ <p> قرار بده. حداکثر ۳ تا ۴ خط در هر پاراگراف. اگر نیاز به لیست بود از <ul>/<li> استفاده کن. خروجی فقط کدهای HTML بدنه باشد."
                ),
            },
        ]
